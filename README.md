...
- **Interface**: the wheel is a main wheel plus pages read from what you carry, all strung together by one continuous
  ...
  off hand and artifacts, opening another page whenever one is not enough; plus resource bars and an aura HUD.
...
| Wheel and Character UI          |   ✅    | Abilities (artifact skills included) and spirit power share one wheel: the main wheel's twelve cells are yours to arrange, the pages behind it follow what you carry, and holding `R` uses the cell you point at.                            |
...
| Techniques                      |   🚧   | Players can learn cultivation techniques to gain active moves or passive bonuses, and exclusion tags can stop certain techniques from being learned together; the mod generates a jade slip per technique. |
...
| Artifacts                       |   🚧   | Items become artifacts via `artifact`: `items` claims them, `spirit_capacity` sets a per-aura ceiling, and `abilities` names what carrying it grants by ability id or tag. Flight and storage are ordinary ability types; refining is missing.                                       |
