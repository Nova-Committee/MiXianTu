...
| Tribulations                    |   ✅    | A tribulation can be triggered on a realm breakthrough: it consumes a timeline of beats (an action, an idle wait, or a wait for a condition), gets harder with the realm and the local aura, and success or failure each run their own outcome. A definition may also ask for a start-up wind-up (its countdown sits on the action bar) and decide whether the sky darkens while it lasts; `/mxt tribulation` runs one by hand and reports how far it has got.                                                          |
