...
public final class FormationService {
    ...
    public static MaintainResult maintain(FormationInstance instance, Formation definition, ResourceHolderAttachment resources,
        ...
        Map<Identifier, Double> capacity = definition.storage()
    ...
    public static final class MaintainRule {
        ...
        /**
         * The capacity of each resource a formation's storage declaration names, evaluated once per period.
         * A capacity that comes out non-finite, zero or negative is dropped rather than clamped: it means
         * this resource is not stored.
         */
        public static Map<Identifier, Double> capacities(Storage storage, FormulaContext context) {
            Map<Identifier, Double> capacities = new LinkedHashMap<>();
            storage.capacity().forEach((resource, provider) -> {
                Identifier id = HolderHelper.idOrNull(resource);
                if (id == null) return;
                double capacity = provider.evaluate(context);
                if (Double.isFinite(capacity) && capacity > 0.0D) capacities.put(id, capacity);
            });
            return capacities;
        }
         ...
         * @param capacity what the stock may hold, per resource; a resource it does not name is not stored
         */
        public static PaymentPlan plan(Formation definition, FormulaContext context,
                                       Map<Holder<Resource>, Double> supplied,
                                       Map<Identifier, Double> stored, Map<Identifier, Double> capacity) {
            Map<Identifier, Double> cost = ResourceTransactions.evaluate(definition.maintenanceCosts(), context).amounts();
            Map<Identifier, Double> fromStock = new LinkedHashMap<>();
            Map<Identifier, Double> fromPayer = new LinkedHashMap<>();
            // What this period's own supply was actually spent on, so the rest of it can be banked. A
            // resource no cost names has spent nothing, so the wrong kind of aura still fills the bank.
            Map<Identifier, Double> spentSupply = new LinkedHashMap<>();
            cost.forEach((id, amount) -> {
                Holder<Resource> resource = MxtDatapackRegistries.holder(MxtResourceKeys.RESOURCE, id).orElse(null);
                // An unresolvable resource absorbs nothing and banks nothing: it keeps the full charge.
                double here = resource == null ? 0.0D : Math.max(0.0D, supplied.getOrDefault(resource, 0.0D));
                double used = Math.min(amount, here);
                if (used > 0.0D) spentSupply.put(id, used);
                double left = amount - used;
                double banked = Math.min(left, Math.max(0.0D, stored.getOrDefault(id, 0.0D)));
                if (banked > 0.0D) fromStock.put(id, banked);
                double owed = left - banked;
                if (owed > 0.0D) fromPayer.put(id, owed);
            });
            Map<Identifier, Double> deposit = new LinkedHashMap<>();
            supplied.forEach((resource, amount) -> {
                Identifier id = HolderHelper.idOrNull(resource);
                if (id == null || amount == null || !Double.isFinite(amount)) return;
                double surplus = amount - spentSupply.getOrDefault(id, 0.0D);
                if (surplus <= 0.0D) return;
                double room = capacity.getOrDefault(id, 0.0D) - stored.getOrDefault(id, 0.0D);
                double banked = Math.min(surplus, Math.max(0.0D, room));
                if (banked > 0.0D) deposit.put(id, banked);
            });
         ...
         */
        public record PaymentPlan(Map<Identifier, Double> fromStock, Map<Identifier, Double> fromOwner,
                                  Map<Identifier, Double> deposit) {
            public PaymentPlan {
