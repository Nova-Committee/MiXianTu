 ...
 * unchanged.
 * <p>
 * These four values must be set before the bolt enters the level: the client's copy is built from the
...
public final class ColoredLightningBolt extends LightningBolt {
    ...
    /**
     * How many gradient stops one bolt may carry. The bound exists for the synched data channel, not for the
     * renderer: a palette is read once per seam, so its length never costs anything at render time.
     */
    ...
    /**
     * Rejects an oversized or non-RGB palette rather than letting the renderer sample something impossible,
     * and copies the list because synched data keeps the reference it is handed.
     */
