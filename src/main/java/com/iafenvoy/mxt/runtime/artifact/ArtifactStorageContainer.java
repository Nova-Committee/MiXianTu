 ...
 * open, and writing into a stack nobody carries any more is how items disappear - so every read and write resolves
 * the artifact from its carrier again, and {@link #stillValid} answers false the moment it is gone, which is what
 * makes the server close the menu (a chest menu asks its container, and the server asks its menu once a tick).</p>
 *
 * <p>Server side only: the client's menu gets a plain mirror of the same size and reads its slots from the menu
 * sync, so nothing here runs where there is no inventory to write into.</p>
 */
public final class ArtifactStorageContainer extends SimpleContainer {
    ...
    /** Whether the artifact is still on this player; a menu that says no is closed by the server. */
    @Override
