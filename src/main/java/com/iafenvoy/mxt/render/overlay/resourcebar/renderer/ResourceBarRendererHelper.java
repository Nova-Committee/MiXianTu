...
final class ResourceBarRendererHelper {
    static void decorations(ResourceBarRenderer.Context context, boolean showValueDisplay) {
    ...
    static void value(ResourceBarRenderer.Context context, int color, boolean showMaximum, String valueFormat) {
        ResourceBarRenderState state = context.state();
        ...
        context.graphics().text(context.minecraft().font, value,
                ...
                context.y() + (state.renderData().height() - 8) / 2, color, true);
    }

    private static void renderIcon(ResourceBarRenderer.Context context, ObjectIntPair<Identifier> icon) {
        ResourceBarRenderState state = context.state();
    ...
    private static void renderName(ResourceBarRenderer.Context context, Component name) {
        ResourceBarRenderState state = context.state();
        ...
        context.graphics().text(context.minecraft().font, name, x,
                context.y() - 2 + Math.max(0, (state.renderData().height() - 8) / 2), 0xFFFFFFFF, true);
    }

    private static void renderValueDisplay(ResourceBarRenderer.Context context) {
        ResourceBarRenderState state = context.state();
        ...
        if (!value.isBlank()) context.graphics().text(context.minecraft().font, value,
                context.x() + (state.renderData().width() - context.minecraft().font.width(value)) / 2,
                context.y() + (state.renderData().height() - 8) / 2, 0xFFFFFFFF, true);
    }
