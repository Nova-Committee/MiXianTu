...
public record ModifyComponentEntityAction(Identifier ability, String component, NumberProvider value) implements EntityAction {
    public static final MapCodec<ModifyComponentEntityAction> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Identifier.CODEC.fieldOf("ability").forGetter(ModifyComponentEntityAction::ability),
    ...
    @Override public void execute(Entity entity, FormulaContext context) {
        ...
        if (holder.has(this.ability)) holder.setComponentState(this.ability, this.component, AbilityComponentState.initial(value, entity.level().getGameTime()));
    }

    @Override public MapCodec<ModifyComponentEntityAction> codec() { return CODEC; }
}
