...
public record ApplyEffectEntityAction(MobEffect effect, NumberProvider durationTicks, NumberProvider amplifier) implements EntityAction {
    ...
    @Override public void execute(Entity entity, FormulaContext context) {
        ...
        if (!Double.isFinite(duration) || !Double.isFinite(amplifier) || duration < 1.0D || duration > Integer.MAX_VALUE || amplifier < 0.0D || amplifier > 255.0D) return;
    ...
    @Override public MapCodec<ApplyEffectEntityAction> codec() { return CODEC; }
}
