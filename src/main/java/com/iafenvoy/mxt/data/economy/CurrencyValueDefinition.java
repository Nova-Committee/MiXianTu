 ...
 */
public record CurrencyValueDefinition(List<Entry> items, long value, List<Exchange> exchanges) implements ItemMatcher {
    public static final Codec<CurrencyValueDefinition> CODEC = RecordCodecBuilder.<CurrencyValueDefinition>create(instance -> instance.group(
            ENTRIES_CODEC.optionalFieldOf("items", List.of()).forGetter(CurrencyValueDefinition::items),
    ...
    ).apply(instance, (items, legacyItem, value, exchanges) -> new CurrencyValueDefinition(
            items.isEmpty() ? legacyItem.map(item -> List.of(Entry.item(item))).orElse(List.of()) : items, value, exchanges
    ))).validate(CurrencyValueDefinition::validate);
