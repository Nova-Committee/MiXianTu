 ...
 */
public record ResourceBarView(double current, double maximum, long ticksSinceChanged, boolean hasAbility) {
    public ResourceBarView {
        if (!Double.isFinite(current) || !Double.isFinite(maximum) || maximum < 0.0D || ticksSinceChanged < 0L)
