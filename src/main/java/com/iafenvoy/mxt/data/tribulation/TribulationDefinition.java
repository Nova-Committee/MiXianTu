 ...
 */
public record TribulationDefinition(List<CultivationCondition> triggerConditions,
                                    List<Phase> phases, NumberProvider difficultyScale,
                                    Optional<Identifier> successBehavior,
                                    Optional<Identifier> failureBehavior) {
    public static final Codec<Holder<TribulationDefinition>> HOLDER_CODEC = RegistryFixedCodec.create(MxtRegistryKeys.TRIBULATION);
    public static final Codec<TribulationDefinition> CODEC = RecordCodecBuilder.<TribulationDefinition>create(instance -> instance.group(
            AutoIgnoreListCodec.create(MxtTypeRegistries.CULTIVATION_CONDITION.byNameCodec()).optionalFieldOf("trigger_conditions", List.of()).forGetter(TribulationDefinition::triggerConditions), Phase.CODEC.listOf().fieldOf("phases").forGetter(TribulationDefinition::phases),
