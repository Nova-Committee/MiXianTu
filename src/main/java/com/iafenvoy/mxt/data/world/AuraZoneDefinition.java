 ...
 */
public record AuraZoneDefinition(double baseAura, double regenPerTick, Map<Identifier, Double> elementAura,
                                 List<Identifier> environmentTags,
                                 List<Either<ResourceKey<LevelStem>, TagKey<LevelStem>>> dimensions,
                                 List<Either<Holder<Biome>, TagKey<Biome>>> biomes,
                                 Fluctuation fluctuation, Rules rules, double elementFitBonus,
                                 double elementConflictPenalty,
                                 Noise noise, ClientRender clientRender) {
    public static final Codec<Holder<AuraZoneDefinition>> HOLDER_CODEC = RegistryFixedCodec.create(MxtRegistryKeys.AURA_ZONE);
    public static final Codec<AuraZoneDefinition> CODEC = RecordCodecBuilder.<AuraZoneDefinition>create(instance -> instance.group(
            Codec.DOUBLE.optionalFieldOf("base_aura", 0.0D).forGetter(AuraZoneDefinition::baseAura),
            Codec.DOUBLE.optionalFieldOf("regen_per_tick", 0.0D).forGetter(AuraZoneDefinition::regenPerTick),
            Codec.unboundedMap(Identifier.CODEC, Codec.DOUBLE).optionalFieldOf("element_aura", Map.of()).forGetter(AuraZoneDefinition::elementAura),
            Identifier.CODEC.listOf().optionalFieldOf("environment_tags", List.of()).forGetter(AuraZoneDefinition::environmentTags),
            RegistryCodecs.keyOrTagList(Registries.LEVEL_STEM).optionalFieldOf("dimensions", List.of()).forGetter(AuraZoneDefinition::dimensions),
            RegistryCodecs.holderOrTagList(Registries.BIOME).optionalFieldOf("biomes", List.of()).forGetter(AuraZoneDefinition::biomes),
            Fluctuation.CODEC.optionalFieldOf("fluctuation", Fluctuation.NONE).forGetter(AuraZoneDefinition::fluctuation),
            Rules.CODEC.optionalFieldOf("rules", Rules.DEFAULT).forGetter(AuraZoneDefinition::rules),
            Codec.DOUBLE.optionalFieldOf("element_fit_bonus", 0.0D).forGetter(AuraZoneDefinition::elementFitBonus),
            Codec.DOUBLE.optionalFieldOf("element_conflict_penalty", 0.0D).forGetter(AuraZoneDefinition::elementConflictPenalty),
            Noise.CODEC.optionalFieldOf("noise", Noise.NONE).forGetter(AuraZoneDefinition::noise),
            ClientRender.CODEC.optionalFieldOf("client_render", ClientRender.DEFAULT).forGetter(AuraZoneDefinition::clientRender)
    ).apply(instance, AuraZoneDefinition::new)).validate(AuraZoneDefinition::validate);

    private static DataResult<AuraZoneDefinition> validate(AuraZoneDefinition value) {
    ...
    public record Noise(boolean enabled, long seed, double scale, double amplitude) {
        ...
        public static final Codec<Noise> CODEC = RecordCodecBuilder.create(instance -> instance.group(
                Codec.BOOL.optionalFieldOf("enable", false).forGetter(Noise::enabled), Codec.LONG.optionalFieldOf("seed", 0L).forGetter(Noise::seed),
                Codec.DOUBLE.optionalFieldOf("scale", 64.0D).forGetter(Noise::scale), Codec.DOUBLE.optionalFieldOf("amplitude", 0.0D).forGetter(Noise::amplitude)
