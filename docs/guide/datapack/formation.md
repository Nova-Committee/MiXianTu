...
服务端配置「兼容」标签页里的 `config.mxt.server.compat.delegate_requires_claims`（**默认开启**）决定没有领地保护时怎么办：
...
`config.mxt.server.compat.claim_linkage`
...
`config.mxt.server.compat.wards_need_claim_permission`（**默认开启**，与上面那个枚举相互独立）
...
- 服务端配置 `config.mxt.server.formation.respect_friends`（默认**开启**）是总开关：关掉之后 `spare_friends` 形同不存在，
...
（需为阵主或管理员；打开服务端配置「阵法 → 队友可拆除」`config.mxt.server.formation.teammates_can_dismantle` 后，阵主的好友也能拆）。
...
- 关掉 `config.mxt.server.formation.plate_auto_detect`（默认开启）就回到旧行为：未绑定的阵盘提示「阵盘尚未绑定阵法」。
