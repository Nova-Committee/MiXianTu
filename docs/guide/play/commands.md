...
`/hud` 存在的理由是**让"编辑器里什么都没有"变成一句能回答的问题**：`/hud` 打出"没有任何可移动元素"就说明元素根本没登记，打出 `resource_bars.left/right` 与 `wheel.selection` 这几行则说明框架是有元素的、只是当前没有内容可画（没有资源条的存档里那两行会是 `块 0`，尺寸仍是空列的 71×8；轮盘没选中过时 `wheel.selection` 也是 `块 0`）。这两种情况的界面表现一模一样，只有这里能分开。
