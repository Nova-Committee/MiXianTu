...
| `/wheel` | 打开轮盘配置界面，等同于按键 `key.mxt.wheel_configuration`（**默认未绑定**）。左边 6 列是能发射的灵气、右边 6 列是已学会的主动技能，下面一排 12 格是轮盘的 12 个扇区；`Esc` 保存并关闭。 |

`/hud` 存在的理由是**让"编辑器里什么都没有"变成一句能回答的问题**：`/hud` 打出"没有任何可移动元素"就说明元素根本没登记，打出 `resource_bars.left/right` 与 `wheel.selection` 这几行则说明框架是有元素的、只是当前没有内容可画（没有资源条的存档里那两行会是 `块 0`，尺寸仍是空列的 71×8；`wheel.selection` 是轮盘格，它整块自绘所以永远是 `块 0`，尺寸固定 94×70）。这两种情况的界面表现一模一样，只有这里能分开。
