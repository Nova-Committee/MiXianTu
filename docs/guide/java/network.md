...
| Payload | 用途 |
| --- | --- |
| `WheelActionC2SPayload` | 轮盘选中一扇：`(kind, id)`。技能与灵气**共用这一条通道**，服务端按 `kind` 分派（技能走 `AbilityService.use`，灵气走 `SpiritBurstService.fireOnce`）。 |
...
| `WheelLayoutC2SPayload` | 轮盘配置界面关闭时把完整的 12 格布局送回服务端；服务端逐格校验后写进玩家附件。 |
| `WheelSelectionC2SPayload` | 换了当前激活的条目（`Optional<WheelSlot>`，空 = 什么都没选）：指针换扇区才发，配置界面保存布局时强制重发一次。服务端用与布局同一套 `WheelEntryKind#exists` 重新解析 id，解析不了就存成"没选中"并在日志里记一条 warning。 |
