...
| `WheelActionC2SPayload` | 轮盘选中一项：`(source, kind, id, enabled)`——**哪个来源**、哪一类、哪个 id，加上一个**可选的**方向。前两个是 **id 字符串**（不是注册对象）：这一侧不认识的页或类型由服务端按"这一项已经失效"拒绝并提示，而不是让包解不出来，所以内容模组注册新的页/类型不必自带协议版本。`enabled` 为空＝"按了这一格"（轮盘走的就是这条，方向归服务端）；填了＝**点名一个状态**的直接请求（脚本或界面用，此时这一项按 id 寻址、不看来源那一页，但技能自己的闸门照过）。`FlightToggleC2SPayload` 已于 2026-09-25 合并进这个字段。 |
