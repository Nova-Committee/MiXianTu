...
**编号指向不存在的格子时自动落到最后一个技能。** `WheelMenuContent#effective(pages, number)` 的规则：

| 情况 | 结果 |
| --- | --- |
| `number < 0`（从没选过） | 没有目标 |
| `number` 在现有格子里、那格有东西 | 就是它 |
| `number` 在现有格子里、那格是空的 | **没有目标**（"指着空格子就是没选"这条老口径不变） |
| `number` 指向**根本不存在**的格子（页没了、或那一页的条目变少了） | **最后一个有东西的格子**（从后往前找） |

**这个回退是刻意的**：物品拿走 → 它的页消失（或页还在但条目变少）→ 编号指向的格子不存在了 → 落到最后一个有东西的格子，而"指着主盘上的空格子按 `V`"仍然什么都不做。**`number` 在任何分支里都不被改写**，所以物品拿回来、格子回来了，同一个编号又指回同一个技能。金框画在 `effective` 上，`V` 花掉的是 `effective`，发出去的编号是 `number`。
...
- **按了没生效时给一句动作栏提示**（2026-09-22 追加，见 `research/27` §9）：只有两种情况——`open(...)` 的"整张轮盘都空"分支发 `actionbar.mxt.wheel.empty`，`use(...)` 里"从没选过任何格子"那一支发 `actionbar.mxt.wheel.no_selection`（带实际绑定的轮盘键名）。**刻意不给提示的第三种情况**：选中的/指针所在的格子当前解析不出条目（空格子、定义被删、授予被撤销）——那在轮盘格上本来就画成空框，按 `V` 也不关轮盘，所以 `usePointed(...)` 的 `selection == null` 分支保持完全静默（第一版曾给它加过 `actionbar.mxt.wheel.no_entry`，用户看过之后点名去掉）。走**客户端** `Minecraft#gui#setOverlayMessage`，不发包——服务端从头到尾不知道有这次按键。**有别的界面打开时也保持沉默**（裸轮询照样读得到物理按键，但那时是在聊天栏打字/翻背包，不是请求）。
