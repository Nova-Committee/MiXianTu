...
- 选中项存在客户端 `WheelSelectionState`（一个 `int`，`-1` = 没选过）里：轮盘打开期间每帧写入指针所在扇区（空扇区也写，因为"指着空的就是没选"比偷偷留着上一扇更诚实），关轮盘后保留，`ClientPlayerNetworkEvent.LoggingIn` 清空，随后由服务端记住的 `selection` 填回来（跨会话那半见上一节）。解析出的条目**每个客户端刻刷新一次并缓存**（`refresh`），因为 HUD 那一格每帧都要读它，而解析要走一遍灵气注册表与"能不能发射"的公式；条目在**用之前**才解析，所以授予被撤销 / 定义被删 / 灵气付不起时那一扇当场变成"没有目标"，按 `V` 什么都不做。
...
- HUD 上新增可拖动元素「**轮盘选中**」（`screen/overlay/wheel/WheelSelectionEntry`，布局键 `wheel.selection`）：画选中那一扇的图标或名字开头、底边一条类型色，不可用时压一层暗色；没选中时只画空槽框。它在 `MiXianTuClient#init` 与资源条一起登记，关着也能按 `V` 这件事靠它才不盲目。
