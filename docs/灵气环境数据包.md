...
- `fluctuation.cycle_type` 支持 `day`、`moon`、`static`。波动只影响查询到的环境浓度；区块库存按 `regen_per_tick × 经过的刻数` 补回，补回周期由 `config/mxt-server.json` 的 `aura.block_aura_tick_interval` 决定（默认每 10 tick）。
