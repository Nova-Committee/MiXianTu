...
- `cultivate_condition` 是此环境允许修炼的实体条件，默认为 `mxt:always_true`。例如 `mxt:aura_range` 可要求当前最终浓度处于 `min..max`；环境条件和当前 `realm_stage.cultivate_condition` 都通过后，修炼才会继续。
...
- 服务端每秒向玩家同步一次区块灵气库存和当前位置环境感知值，以及来源 `aura_zone`。`stored_aura` 只显示区块共享库存，`sensed_concentration` 和雾效只显示当前位置环境模板计算值；环境波动不会直接改写库存显示，但方块贡献、修炼消耗和库存回复仍会改变库存。雾效强度还会按感知浓度缩放，因此低浓度区域会更淡。灵气粒子仍按每 5 tick 刷新。
