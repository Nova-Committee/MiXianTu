...
- **契约兽的命令是"类 + 装载类"，不是枚举**：`data/creature/ContractBehavior` 是一条命令（`id` + `momentary` + `name()`），框架内置的跟随 / 游荡 / 驻守 / 召回装在 `ContractBehaviors` 里，内容方 `new` 一个再 `register(...)` 就多一条，不去扩枚举。命令由**实体自己回答**（`ContractOperations.behaviors()` / `onBehaviorSelected`（输入端）/ `tick`（每 tick 驱动）），当前那一条**只有一份**（`mxt:contract` 附件，存 id，读不出来退回跟随），输入端唯一出口是 `runtime/creature/ContractBehaviorService`（御兽铃轮盘的 `WheelSource.CONTRACT` 与 `/contract behavior` 都走它）；铃只是**指针**（组件 `mxt:contract_bell`），轮盘页读组件而不去解析客户端上可能没加载的实体。口径见 `AGENTS.md` 第 4 节。
