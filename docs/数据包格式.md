...
全模组的图标字段都是同一个值，**直接内联**：贴图写成一个字符串，物品写成一个对象。带这个字段的定义有 `ability`、`resource`、`badge`、`forging_method` 与 `cultivation_technique`。
...
下表列出 `MxtDatapackRegistries` 注册的 35 个数据包注册表。字段表中的“默认”是 Codec 默认值；“必填”表示缺失时加载失败。表格末尾的 `alchemy_recipe` 不是数据包注册表，而是使用原版配方系统的配方类型，一并列在此处便于查阅。
...
| --- | --- | --- | --- |
| `mxt:empty` | 无 | - | 空占位徽章。 |
| `mxt:sprite` | `icon` | **必填** | 只显示图标。 |
| `mxt:tooltip` | `icon`、`text` | `text` 为空文本 | 带悬浮说明的图标。 |
| `mxt:keybind` | `icon`、`text`、`key` | `text=""`, `key="key.mxt.primary_active"` | 记录客户端按键映射。 |
| `mxt:crafting_recipe` | `icon`、`recipe`、`from_power`、`prefix`、`suffix` | `from_power=false` | 预留配方提示元数据。 |

`icon` 是**图标引用**（见「图标引用」一节），所以徽章既能用贴图也能用物品。

```json
{
  "type": "mxt:tooltip",
  "icon": "mxt:textures/gui/badge/info.png",
