...
- 秘境实例：`realm_instance_members`、`realm_instance_limit`（不限时为 `-1`）、`realm_instance_elapsed`、`realm_instance_duration`、`realm_instance_index`、`realm_instance_is_owner`（`1` 或 `0`）。不在任何秘境实例里时这些名字无法提供，按错误处理而不是静默为 `0`。
...
秘境定义是**一份模板**，不是某个固定维度：每次进入都会按它开出一份**实例维度**，实例的身份就是那个维度键。维度键固定为 `<定义命名空间>:realm/<定义路径>/<序号>`，序号从 `0` 开始——**即使这份定义只能开出一份实例，序号也在**（`example:realm/trial_realm/0`）。这样每份实例的地形天然隔离在 `dimensions/<命名空间>/realm/<路径>/<序号>/` 下，灵气区域也能直接按这个 id 命中（见本节末尾）。
...
- 实体条件 `mxt:in_realm_instance` 判定"我是否在某份实例里"，`role` 取 `any`（在里面）、`owner`（我是主人）、`guest`（在里面但不是主人）；可选 `definition` 字段把判定限定到某个定义或标签。
- 公式变量 `realm_instance_is_owner`（`1` 或 `0`）、`realm_instance_members`、`realm_instance_limit`、`realm_instance_elapsed`、`realm_instance_duration`、`realm_instance_index`，可在进出条件与进出行为里读。不在任何实例里时这些名字按"无法提供"处理（开发环境会报日志）。名字带 `realm_instance_` 前缀是因为 `realm` 已经被境界占用了。
...
**灵气整合。** 实例维度是运行时维度，没有自己的 `LevelStem` 注册项，所以 `aura_zone.dimensions` 除了直接写实例维度 id（`<命名空间>:realm/<路径>/<序号>`），还可以写该实例所用的 `stem`（如 `minecraft:the_end`）或定义 id——两者都会命中它的**所有份**。"末地型秘境统统是死灵气"这类规则因此只需要一条区域。
...
**删除定义前先清理实例。** 实例记录用注册表 holder 持久化定义引用，所以如果删掉一个**还有实例存在**的秘境定义，下次启动时那份记录会读不出来（找不到条目），它的地形会被当成遗留数据清掉。先 `/mxt realm_instance destroy <维度>`，再删定义。
