...
全模组的图标字段都是同一个值，**直接内联**：贴图写成一个字符串，物品写成一个对象。带这个字段的定义有 `ability`、`resource`、`forging_method` 与 `cultivation_technique`。
...
| `cultivation` | `mxt/cultivation` | 单个数值的修炼行为档案（境界入口、恢复、灵气、换算、可用性）。 |
...
| `talisman` | `mxt/talisman` | 符篆定义：一张符篆铭刻的能力。 |
...
`resource` 只是一个按实体存储的数值：它的边界和显示方式。它不关心数值来自哪里、做什么用——把数值变成修炼资源的那些内容（境界链入口、自然恢复、灵气标记、修为换算、可用性门禁）都在 `cultivation` 里，并通过 `resource` 字段指回该数值。没有对应档案的 resource 就是一个普通计数器。
...
数值始终被钳制在 `[min, max]`，越界变更会被拒绝。数值可以作为 `ResourceCost` 被消耗、用 `mxt:resource_compare` 比较、在公式里以 `caster_<名称>` 读取、通过灵力访问接口存入物品，并用 `mxt:add_resource` 行为增减。
...
文件位置：`data/<namespace>/mxt/cultivation/<id>.json`

一个 `cultivation` 档案描述**一个**数值的修炼行为，与 `resource` 是**一对一**关系：同一个数值最多有一个档案，服务器构建缓存时发现重复会直接拒绝。
...
| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `cultivation` | `Holder<cultivation>` | **必填** | 该阶段所属的修炼链。一个阶段只指向一个档案，而档案才指向被存储的数值；链条本身不以数值为键。 |
...
境界附件按 `cultivation` 档案保存当前境界与修为进度（链条的键就是档案本身，读状态时不需要再反查数值）；服务器境界缓存保存“境界 -> 修炼档案”的映射与链内序号。境界链只能通过 `next_realm` 单向推进，链身份是档案而不是数值本身。
...
| `aura_costs` | `Map<Holder<resource>, NumberProvider>` | `{}` | 每次修炼消耗的环境资源；各资源独立分配，某一项不足仅降低该项贡献。 |
...
| `technique` | `Holder<cultivation_technique>` | **必填** | 学习的目标功法。 |
| `quality_group` | `Tag<item_quality>` | 无 | 允许的品质组。 |
...
**判定以服务端为准，姿势不是。** 原版的一次使用周期在客户端和服务端各有一个独立计数器，而服务端不一定跑满 20 TPS——那样服务端的 60 tick 在真实时间里就比客户端的 60 tick 长。模组让**判定**（读完没有、学会没有、进度条）走服务端，所以 `learn_time` 是"服务端需要跑满的 tick 数"，服务端掉帧时会相应变长，这是正确的行为。**手臂姿势走的是客户端自己的计数**，所以服务端掉帧或延迟较大时，姿势可能比判定早一点收掉：那段尾巴通常只有一两 tick，正常服务器上看不出来，只有服务端明显跟不上时才会看到"动画断了一下、功法随后才到"。模组不再为此修改客户端；真遇到时客户端日志里会有 `[mxt] hold pose lost with N of the hold left`（`HoldPoseDiagnostic`），可以用它确认是不是这件事。
...
| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `aura` | `Map<Holder<resource>, AuraValue>` | `{}` | 每种资源独立的环境库存定义；`AuraValue` 包含数量、上限、恢复速度和颜色。 |
...
| `aura` | `Map<Holder<resource>, AuraValue>` | `{}` | 每个方块提供的资源数量、容量、恢复速度和颜色。 |
...
| `type` | `Holder<resource>` | **必填** | 消耗和释放的资源类型；元素只通过该数值 `cultivation` 档案里的 `aura_type` 参与类型判断。 |
...
| `storage` | `Storage` | 无（不启用） | 阵法自己的存量：`capacity`（`Map<resource, NumberProvider>`，必填）声明每种资源最多存多少。存下自家地脉供应的盈余，付账顺序变成 地脉 → 存量 → 阵主。通用字段，任何阵法都能写。 |
...
| `minimum_aura` | `Map<Holder<resource>, NumberProvider>` | `{}` | 生成或强化所需的各资源最低值。 |
...
一张符篆的定义：铭刻到载体上之后，激发它授予哪些能力、要付多少灵气。

**效果只有 `abilities` 一个字段**——本模组里所有“会做事的技能性效果”都落在 `ability` 上，符篆因此不需要自己的一套效果词汇。制符闭环落地后还需要的东西（使用次数、触发条件）会以**可选**字段加入，今天写好的文件继续可解析。

| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `abilities` | `HolderOrTag<ability>[]` | `[]` | 该符篆授予的能力，写法与 `cultivation_technique.granted_abilities` 一致：单个 ID、ID 数组或 `#标签`。 |
| `aura_cost` | `Map<Holder<resource>, NumberProvider>` | `{}` | 激发该符篆要付的灵气账单，按具体 resource 分键，每项可以写常量或公式。缺省或为空表示这张符不耗灵气。 |

`aura_cost` 只收具体 ID，**不收 `#标签`**，和 `abilities` 相反：灵气池本身就是按具体 resource 分池的（见 `aura_zone` 的 `aura`），标签在这里找不到对应的池子。数值写法与其它灵气消耗一致（`cultivate_action.aura_costs`、`formation.storage.capacity`），因此可以写成 `"12"` 或 `"realm_rank * 4"`。

**目前没有任何东西收这笔账**：激发符篆的服务还没写，`aura_cost` 只是先把账单声明出来。
...
“已经铭刻了哪些符篆”由物品组件 `mxt:talisman` 保存：它是一个按追加顺序排列的 `talisman` 条目列表，空列表就是刚做出来的空载体。组件里存的是**具体条目**，因此不能写标签。组件与载体的说明见 `docs/通用物品.md`。
...
玩家已学会的功法以**引用**形式存在玩家数据里。当数据包**删除或禁用**某个 `cultivation_technique` 后，存档里的旧引用就指向了一个不存在的定义。
