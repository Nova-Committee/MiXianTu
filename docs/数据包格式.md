...
`mxt:formula_variable` 是**固有注册表**，数据包不能添加条目：每个条目负责从 `FormulaContext` 携带的对象里"拆出"一个数字——施法者、目标、资源对象或随机源。变量按需读取，公式只为自己真正用到的名字付出代价；事件载荷（`damage`、`block_x` 等）不属于任何对象，仍由调用方写入上下文的显式值表。
