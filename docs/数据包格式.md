...
- 数据驱动定义的显示名称由标识符自动生成翻译键：`Identifier.toLanguageKey` 按注册表类别生成 `<category>.<namespace>.<path>`（路径中的 `/` 转为 `.`），例如 `example:qi` 生成 `resource.example.qi`。JSON 不再填写 `translation_key`，请在 `assets/<命名空间>/lang/zh_cn.json` 和 `en_us.json` 中提供对应翻译。
...
| `mxt:spirit_storage` | 无 | 所有实现 `AuraItemAccess` 的物品，即能被按住右键灌注灵气的物品（见 [物品灵气数据包](物品灵气数据包.md)）。这是唯一一个按「能力」而不是按 ID 匹配的条目，因此之后新增的同类物品会被自动覆盖。 |
...
- 其他系统：契约战斗行为提供 `damage`；阵法 `entity_tick_action` 提供 `formation_radius` 与 `distance`；天劫阶段提供 `aura_tribulation_modifier`。
...
所有行为的具体字段以固有类型 Codec 为准。内置类型由 `MxtEntityActions`、`MxtBiEntityActions`、`MxtBlockActions`、`MxtItemActions`、`MxtEntityConditions` 等类分组注册；数据包不会向这些固有注册表添加条目。
...
`Trigger` 的 `mxt:js` 必须额外声明 `signal`，因为运行时按信号索引订阅。所有 `mxt:js` 回调都在服务端执行；除 `Cost` 之外，回调都能拿到本次派发的公式上下文——`Cost` 只用玩家求值，因此它的上下文仅由该玩家构建，不含事件载荷。战利品条件与函数写在原版战利品表里（`condition` / `function` 分派键），同样只在服务端生成战利品时执行。

数据包无法向固有注册表**新增 `type`**，只能选择已注册的类型；`mxt:js` 是其中唯一能把行为交给脚本的类型。

## 动态注册表

下表列出 `MxtDatapackRegistries` 注册的 34 个数据包注册表。字段表中的“默认”是 Codec 默认值；“必填”表示缺失时加载失败。表格末尾的 `alchemy_recipe` 不是数据包注册表，而是使用原版配方系统的配方类型，一并列在此处便于查阅。
...
数值始终被钳制在 `[min, max]`，越界变更会被拒绝。数值可以作为 `ResourceCost` 被消耗、用 `mxt:resource_compare` 比较、在公式里以 `caster_<名称>` 读取、把**该值所对应的灵气**（`Aura` 定义）通过存取接口（`AuraAccess`/`AuraItemAccess`）存入物品，并用 `mxt:add_resource` 行为增减。
...
文件位置：`data/<namespace>/mxt/aura/<id>.json`
...
文件位置：`data/<namespace>/mxt/tool_binding/<id>.json`
...
文件位置：`data/<namespace>/mxt/blueprint_binding/<id>.json`
...
文件位置：`data/<namespace>/mxt/skill_stage/<id>.json`
...
| `damage_multiplier` | Double | `1.0` | 该水平的伤害倍率，必须为有限且非负的数值。 |

`mastery` 属于水平本身而不是某个功法：共用一条链的持有者面对的是同一段爬升。它可以是公式，但链上任何一处"下一级要求比上一级更低"都会被拒绝。

> `damage_multiplier` 目前**没有被消费**：字段会被加载与校验，但运行期没有任何读取点，因为没有统一的技能伤害管线（伤害来自 `mxt:damage` 动作里的 `NumberProvider`）。可以先把数据写上，等伤害管线出现后才会真正生效。
...
文件位置：`data/<namespace>/mxt/technique/<id>.json`
...
物品冷却由**服务端配置**控制，不在数据包里：

```json
// config/mxt-server.json
{
  "cultivation": {
    "technique_learn_cooldown": 60
  }
}
```

单位 tick，范围 `0..72000`，默认 `60`（3 秒），写 `0` 关闭。**无论学会成功与否，一次读完的阅读都会进冷却**——冷却是"这次尝试"的代价，不是失败的惩罚：它要挡的是同一本手册被反复读，而"被拒绝不花冷却"恰好把这条路径留给任何当前学不会的功法。走的是原版 `Player#getCooldowns`，所以热键栏的灰色扫描和 `mxt:on_cooldown` 物品条件都能直接读到。
...
实现 `AuraItemAccess` 的物品（灵石、符箓载体等）另有一份自己的存储组件 `mxt:spirit_storage`：一张「灵气 → 已存单位」的表（`{amounts:{"mxt:common":100}}`，键是 `mxt:aura` 注册表条目），灵石与符箓共用。`type` 只声明该物品**接受**哪一种灵气，容量由 `aura` 给出（符箓则由 `aura_cost` 账单给出）；已经装过东西的物品以自己记下的灵气键为准，因此数据包事后改掉 `type` 不会把世界里已有的存量改读成另一种灵气，只会让它与新 `type` 对不上（装不进、也烧不出）。灵石一次只装一种、读它唯一的那条记录；空表表示被抽空，缺组件的物品视为满充。
...
在施法者位置打下一道雷。除了颜色，它**就是原版闪电**：伤害、引燃、避雷针充能、铜氧化、雷声、天空闪光以及村民→女巫、猪→僵尸猪灵、苦力怕充能这些雷击转化全部照旧。这是一个 `EntityAction`，所以天劫阶段、成败行为、阵法逐实体行为、技能、契约、秘境——任何 `EntityAction` 槽位都能用。
...
不做数据包、只想直接打一道雷时用 `/mxt lightning`（顶层别名 `/lightning`），参数与上表一一对应，见[命令](../guide/play/commands)。
...
文件位置：`data/<namespace>/mxt/trigger/<id>.json`
...
文件位置：`data/<namespace>/mxt/talisman/<id>.json`
