...
| `ItemStack` | `{"id":"minecraft:amethyst_shard"}` | 原版物品堆，必须写成对象（`id` 必填，可带 `count`、`components`），**不接受裸物品 ID 字符串**；与模板的区别就在这里。 |
...
| 类型 | 专属字段 | 作用 |
| --- | --- | --- |
| `mxt:flight` | `speed`（必填 `NumberProvider`）、`display`（载具怎么画，默认＝"放平 + 剑刃朝前 + 两倍大"） | 载人飞行：`speed` 是载具速度，每 tick 的费用就是这个技能自己的 `costs`。它同时是"需要按键"的（`Toggable`），所以会作为一格进轮盘：按一下起剑、再按一下落剑；**落地不收费**，否则付不出费的人会被困在空中。**承载者可以是任何生物**（2026-09-25 起）：玩家的入口是轮盘那一格，生物身上的 `mxt:flight` 由脚本、战利品函数或其它行为驱动——只有"记住并恢复玩家自己的飞行权限"那一步是玩家专属。 |
...
**需要按键的法器技能（`Toggable`）。** 技能类型凡是实现了 `Toggable` 接口的就是**需要按键才能发动**的：实现它就等于声明"把这一项放进轮盘"（设计见 `research/40_能力与法器能力合并设计.md`，同日的两次收缩在其 §12）。今天有三个实现，都是普通技能类型、都不用新字段：`mxt:active`（原本就按一下施放）、`mxt:flight`（开关：开＝起剑、关＝落剑）、`mxt:storage`（一次性：打开储物箱，没有状态）。约定如下：
...
- **状态归实现自己管**，不是数据包字段：`mxt:flight` 读的是**承载者**那份飞行状态（记着"飞的是哪条技能"），`mxt:storage` 没有状态（格子里就是"按一下打开"）。轮盘只显示状态（开关底色绿＝开、灰＝关；一次性的是紫色），按下去由**服务端**重读一遍再决定做什么，所以请求本身不携带"开到哪一侧"。
- 接口还要求实现把失败原因说清楚：`Result(changed, failure, failedResource)`，`Failure` 共 15 个取值（`NOT_OWNED` / `ALREADY_SET` / `UNAVAILABLE` / `NO_CARRIER` / `CANNOT_MOUNT` / `NOT_GRANTED` / `COOLDOWN` / `INSUFFICIENT_RESOURCE` / `INSUFFICIENT_COST` / `INVALID_FORMULA` / `CONDITION_FAILED` / `NO_CHARGES` / `CANCELLED` / `PERMISSION_DENIED` / `ELEMENT_AFFINITY`），名字与 `AbilityService.Failure` 对齐——**2026-09-23 起按压与施放共用 `actionbar.mxt.ability.failure.*` 一份文案表**，所以"条件不满足 / 灵根不符 / 次数用完 / 数值配置有误"这些原因会照实报出来，不再被压成一句「现在用不了」（`UNAVAILABLE` 只剩兜底：拿不到服务端玩家、飞行 `startRiding` 失败、按下的东西根本不是 `Toggable`）；付不出某种资源时 `failedResource` 也带上是哪一门。轮盘把原因同时报在动作栏与日志里，日志带原因名与缺失资源。这一格叫什么用的是技能自己的 `name`，接口不再另给一个名字。
...
| `inner_core` | `ItemStack` | 无 | 内丹物品堆，写 `{"id": "..."}`，可带 `count`（`1..99`）与 `components`；生物死亡时按这一堆掉落。 |
