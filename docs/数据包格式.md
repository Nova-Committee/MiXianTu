...
| `description` | Text Component | 无 | 品质说明。 |
| `value_multiplier` | `NumberProvider` | `1` | 货币价值倍率。 |
| `forging_modifier` | `NumberProvider` | `1` | 锻造修正倍率。 |
| `alchemy_modifier` | `NumberProvider` | `1` | 炼丹修正倍率。 |
...
自然环境使用 `max(0, (base_aura + noise) / 10 - 5)` 初始化，噪声由 `seed`、`scale`、`amplitude` 控制；方块灵气会在此基础上累加。环境优先级为群系 < 维度 < 永久区域 < 阵法覆写。
...
| `total_aura` | `NumberProvider` | **必填** | 消耗一件物品时装入隐藏燃料值的总量。 |
| `consume_speed` | `NumberProvider` | **必填** | 每 tick 消耗燃料值。 |
| `result_stack` | `ItemStackTemplate` | 无 | 配置后额外给予返还物品；不配置则只扣原物品。 |
| `exhausted_action` | `EntityAction` | `mxt:no_op` | 当前燃料耗尽行为。 |

修炼模式下燃料值为空时，按主手再副手寻找匹配物品并装入；燃料值属于 `SpiritData` 附件，服务端扣除和同步，客户端不渲染燃料条。
