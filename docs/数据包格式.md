...
**需要按键的法器技能（`Toggable`）。** 技能类型凡是实现了 `Toggable` 接口的就是**需要按键才能发动**的：实现它就等于声明"把这一项放进轮盘"（设计见 `research/41_能力与法器能力合并设计.md`，同日的两次收缩在其 §12）。今天有三个实现，都是普通技能类型、都不用新字段：`mxt:active`（原本就按一下施放）、`mxt:flight`（开关：开＝起剑、关＝落剑）、`mxt:storage`（一次性：打开储物箱，没有状态）。约定如下：
