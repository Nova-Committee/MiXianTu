...
| `actions` | `List<Formation Action>` | `[]` | 功能模块列表：`mxt:attack`、`mxt:buff`、`mxt:protection`、`mxt:none`。灵气覆写 `aura_zone` 与上限加成 `max_bonus` 属于 `mxt:buff` 模块的字段；把守御交给领地插件用 `mxt:protection` 的 `delegate_to_claims`。 |
| `hostile` | `boolean` | `false` | 逐实体行为是否用于伤害；带 `mxt:attack` 模块时不需要声明。 |
