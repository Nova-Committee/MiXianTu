...
- 用这个数组的字段：`ability.costs`、`mxt:channelled` 的 `upkeep_costs`、`realm_stage.costs`、`cultivate_action.costs` 与 `aura_costs`、`formation.activation_costs` 与 `maintenance_costs`、`forging_method.costs`、法器 `mxt:flight` 与 `mxt:upkeep` 的 `costs`、灵气合成配方的 `aura`。其中 `cultivate_action.aura_costs` 与配方的 `aura` **只收 `mxt:aura` 条目**（写别的类型会在加载期被拒），老写法「灵气 → 数量」的 map 仍然读得进去，写出去的一律是数组。
...
法器不是一种新物品，而是**已有物品的一套规则**：`items` 声明这份定义认领哪些物品（与 item/weapon/pill 三种 binding、`spirit_herb` 完全同一套 `ItemMatcher`），匹配到的物品堆就是这件法器。它做什么由 `abilities` 逐条声明，每条按自己的 `type` 走固有分派。定义**没有**"器型"这类自由标签字段（旧的 `item_type` 已删除，老定义里写着它也只是被**静默忽略**）：**它自己的注册表 id 就是这件法器的名字**，要"把一族法器归到一起"就用物品标签（`items` 里的 `#标签`）或 id 的命名空间与路径来表达。
...
| `type` | 字段 | 作用 |
| --- | --- | --- |
| `mxt:empty` | 无 | 什么都不做的占位类型（这张固有注册表的默认项）。`type` 本身仍是必填字段。 |
...
| `mxt:flight` | `speed`（必填 `NumberProvider`）、`costs`（`List<Cost>`，默认 `[]`）、`display`（载具怎么画，默认＝"放平 + 剑刃朝前 + 两倍大"，见下） | 载人飞行：`speed` 是飞行载具速度，`costs` 是每 tick 的消耗。**一个定义最多一条。** 它同时是一个**法器技能**（`ToggableArtifactAbility` 的实现者），所以会作为一格出现在轮盘上：按一下起剑、再按一下落剑（见下）。 |
| `mxt:storage` | `slots`（必填 `NumberProvider`） | 自带储物槽位，**一个定义最多一条**。格数求值后**按 9 向上取整**（界面是标准箱子：一行九格），并夹在 `6` 行以内，也就是**最多 54 格**——界面打不开的格子就不该存在，所以容量与窗口永远是同一个数（`ArtifactService#storageSlots`）。内容写在物品组件 `mxt:artifact_storage` 上，`ArtifactStorageService` 是唯一读写口（只有主人与服务端能碰）。它同时是一个**法器技能**（见下）：轮盘上那一格按一下就打开这个箱子，窗口标题是法器的名字。 |
| `mxt:upkeep` | `costs`（`List<Cost>`，默认 `[]`）、`interval`（`NumberProvider`，默认 `20`）、`on_fail`（ItemAction，默认 `mxt:no_op`）、`owner_only`（bool，默认 `true`） | **周期性代价**：携带（主手／副手／任意 Curios 槽）期间每 `interval` 刻结算一次，把 `costs` **全部一起**扣掉（全有或全无，不半扣）。付不出时跑 `on_fail`（对持有者与该物品堆执行），既不报错也不静默跳过。`owner_only` 为真（默认）时只有主人承担，未认主＝没人付；为假时谁带着谁付。时钟是**世界时间**：只有能被 `interval` 整除的刻才结算，所以不是"拿到手之后每 N 刻"。**一个定义最多一条。** |

以下九个键**不再被读取**：写了不会报错，也不会生效（`RecordCodecBuilder` 会忽略未声明的键）。其中四个已并入 `abilities`——`granted_abilities` 现在写成 `mxt:passive` / `mxt:active` 条目，`flight_speed` / `flight_costs` 写成 `mxt:flight`，`storage_slots` 写成 `mxt:storage`；四个是认主相关的旧名：`refine_health_cost`（并入 `claim_action`）、`refine_action` → `claim_action`、`refine_condition` → `claim_condition`，以及短暂存在过的 `claim_cost`（也并入 `claim_action`）；最后一个是已整个移除的 `item_type`（器型标签，没有替代字段——定义 id 与物品标签就是它的替代）。老包照着这些名字写会**既不报错也不生效**，于是变成"认主默认扣 4 点血、没有认主效果、条件不再判"——需要手工搬过去。
...
**法器技能（`ToggableArtifactAbility`）。** `abilities` 里凡是实现了这个接口的条目都是**需要按键才能发动**的能力：实现它就等于声明"把这一项放进轮盘"（设计见 `research/32_法器开关与轮盘接线设计.md`）。今天有两个实现，都是固有类型、都不用新字段：`mxt:flight`（开关：开＝起剑、关＝落剑）与 `mxt:storage`（一次性：打开这件法器的储物箱；它没有状态）。约定如下：

- **一件法器可以有好几个**，一个 `key` 一个：轮盘的条目身份是**法器定义 id + 能力 key**（写成 `命名空间:路径/key`，如 `mxt_test:bound_sword/flight`），所以同一把剑可以既有飞行又有储物；同一个 `key` 写两次由加载期拒绝。
- 它出现在**声明它的那张从盘**上（主手 / 副手 / Curios，排在技能之后），也可以被玩家钉到主盘——两处都读"那件法器此刻在不在双手或 Curios 槽里"，不存任何东西。
- **状态归实现自己管**，不是数据包字段：`mxt:flight` 读的是玩家那份飞行状态，`mxt:storage` 没有状态（格子里就是"按一下打开"）。轮盘只显示状态（开关底色绿＝开、灰＝关；一次性的是紫色），按下去由**服务端**问一遍实现再决定做什么，所以请求本身不携带"开到哪一侧"。
- 接口还要求实现给出这一格的名字（`displayName`）与失败原因（不在身上 / 不认你 / 状态已经是这样 / 现在用不了），轮盘把原因报在动作栏与日志里。
- 将来新增"需要按键"的法器能力，就是新增一个实现这个接口的 `mxt:artifact_ability_type` 条目，不用再动轮盘。
...
- 想让"带在身上"持续付代价：在 `abilities` 里加 `{ "type": "mxt:upkeep", "costs": [{ "id": "mxt:qi", "amount": 1 }], "interval": 20, "on_fail": { "type": "mxt:damage_item", "amount": 1 } }`。
