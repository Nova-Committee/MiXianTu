...
| `first_realm` | `Holder<realm_stage>` | 无 | 该资源对应境界链的首个境界；只用于确定凡人首次突破后的目标境界。 |
| `start_exp` | `NumberProvider` | `0` | 凡人首次突破所需修为，同时是凡人阶段的修为上限；达到后不再增加，只能尝试突破。 |
| `start_cultivate_conditions` | `CultivateConditions` | 空对象 | 凡人开始修炼及首次突破前检查的资源级条件，可用于冲突或身份限制。所有带首境界的资源均满足后才能开始修炼。 |
| `use_condition` | `EntityCondition` | `mxt:always_true` | 控制实体是否能主动消耗资源，并决定资源条是否显示；不影响修炼、环境吸收、自然恢复或突破。 |
| `show_cultivation_info` | Boolean | `true` | 是否在角色信息面板显示该资源对应的境界和修为进度。 |

`CultivateConditions` 对象包含 `conditions`、可选 `triggers` 和可选 `action`。开始修炼和突破入口立即检查 `conditions`（全部满足）；达到突破阶段后，当前资源才会按 `triggers` 注册运行时订阅，匹配事件后再尝试突破。订阅不会直接序列化，存档加载或数据包重载后由修炼状态重建。
...
区块加载、方块变化或数据包重载后重建缓存。灵石矿脉可以通过多个 `block_aura` 条目和方块标签提供高于自然环境的灵气。运行时按 7×7×7 子区块范围查询：当前子区块周围 3×3×3 范围使用匹配方块的真实位置，外围使用子区块中心近似，统一按 `1 / max(1, 距离平方)` 衰减；每个来源子区块的贡献会根据当前访问玩家数粗略平分，库存仍是区块范围共享的。
