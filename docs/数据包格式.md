...
法器不是一种新物品，而是**已有物品的一套规则**：`items` 声明这份定义认领哪些物品（与四种 binding、`spirit_herb` 完全同一套 `ItemMatcher`），匹配到的物品堆就是这件法器。它做什么由 `abilities` 逐条声明，每条按自己的 `type` 走固有分派。

| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `item_type` | String | **必填** | 器型标识，不能为空。给数据包与内容方认族用的标签（同一族的法器归到一起）；**代码里目前没有按它的值分流的判断**。 |
...
| `curios_equipable` | bool | `false` | 这份器型是否允许放进 Curios 槽位。腰带槽在 `WEAPONS_ARTIFACTS` 模式下接收匹配 `weapon_binding` 的物品，或声明了 `true` 的法器；四个 `charm` 槽位**只**收声明 `true` 的法器（`curios:charm` 物品标签仍可放别的东西）。 |
...
| `refine_action` | ItemAction | 无操作 | 法器精炼（认主）时对持有者和物品执行的行为。 |
...
以下四个字段**不再被读取**：写了不会报错，也不会生效（`RecordCodecBuilder` 会忽略未声明的键）。它们都已并入 `abilities`——`granted_abilities` 现在写成 `mxt:passive` / `mxt:active` 条目，`flight_speed` / `flight_costs` 写成 `mxt:flight`，`storage_slots` 写成 `mxt:storage`。

灵力与温养：

- 存量写在**共用组件** `mxt:spirit_storage`（按灵气记整单位，与灵石、符箓载体同一个形状），不是法器专属组件。
- 上限来自 `spirit_capacity` 里那一种灵气的声明值；没有声明过的灵气灌不进去（上限按 0）。
- `mxt:artifact_state` 组件现在只有 `owner_uuid`（归属）与 `nourishment`（温养度 `0..1`）。每次真正收下灵气，温养度按**收下的量 ÷ 本次该灵气的有效上限**上涨并夹在 `0..1`，只升不降；有效上限 = `floor(声明上限 × (1 + 0.5 × nourishment))`，养满即 1.5 倍，两端都夹取。想读温养度，用通用的 `mxt:component` 条件读 `mxt:artifact_state` 的 `nourishment`。
- 归属由 `mxt:set_artifact_owner`（战利品函数）或任何调用 `ArtifactService.refine` 的地方写入——**本体里目前只有战利品函数这一条路径**，精炼台与命令都还没接。**飞行与储物按 `require_owner` 判权**：`false`（默认）时未认主的谁都能用、认主后只认主人；`true` 时未认主一律拒绝。`mxt:owned_by` 与 `require_owner` 无关，它问的就是"归属是不是你"（未认主即 false）；授予能力只看是否持有/装备。

物品提示框（Tooltip）会把上面这些声明逐行报出来，所以写进来的数字作者自己就能看见：法器名与 `item_type`、每种灵气的「存量 / 有效上限」（带温养加成，按百分比着色）、`abilities` 的每一条（`mxt:passive`／`mxt:active` 列出能力名，`mxt:flight` 报速度与每刻消耗，`mxt:storage` 报「已用 / 总格数」，`mxt:empty` 不占行），以及认主状态（有主人时显示绿色 `✔ 已认主`；只有 `require_owner: true` 且还没认主时才显示红色 `✖ 未认主 · 需认主后才能飞行与储物`——不需要认主的法器未认主时不占行）；温养度大于 0 时另起一行。按住 F3+H 的**高级提示**还会多出定义 id 与主人 UUID。（`curios_equipable` 不在这里重复：槽位是否接受由 Curios 自己的提示列出。）
