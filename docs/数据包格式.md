...
原理：原版的 use 周期会向**物品**询问"这个动作持续多久"（`getUseDuration`）和"播哪个姿势"（`getUseAnimation`），模组把这两个问题的答案接到 `technique_binding` 上。这两问在客户端和服务端都会被问到，两端各自从**同步到本地的数据包注册表**读出同一份绑定，因此看到的是同一个长按。

**阅读姿势以服务端为准。** 原版的一次使用周期在客户端和服务端各有一个独立计数器，而服务端不一定跑满 20 TPS——那样服务端的 60 tick 在真实时间里就比客户端的 60 tick 长。模组让姿势只在**松手**或**服务端读完**时才结束，不会因为客户端本地计时先到而提前收手；进度条同样以服务端为准。所以 `learn_time` 是"服务端需要跑满的 tick 数"，在服务端掉帧时会相应变长，这是正确的行为。

:::warning
长按**不会消耗**手册：物品本身不是食物，读完之后原样留在手上。但如果该物品**本来就有食用属性**（例如绑到面包上），长按结束时会按原版规则被吃掉——这是该物品自身的行为，模组不去改动它。
:::
