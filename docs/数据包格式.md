...
`context` 使用固有注册表 `mxt:resource_bar_context` 中的 ID。上下文对象负责从实体或客户端状态提取当前值、最小值、最大值和最近变更时间，并使用传入的资源 ID 生成显示名称。例如 `mxt:sensed_concentration` 的翻译模板为 `感受到的%s`，配合 `resource.example.qi=灵气` 会显示为“感受到的灵气”。内置上下文为 `mxt:self_hud`、`mxt:target_overlay`、`mxt:boss_overlay` 和 `mxt:sensed_concentration`。
...
| `aura_costs` | `Map<Holder<resource>, NumberProvider>` | `{}` | 每次修炼消耗的环境资源；键必须是资源 ID。 |
...
自然环境以每个资源的 `amount` 为初始库存，噪声和波动作用于该资源；`AuraValue.max` 省略时默认为初始值。可用固定数值、`{ "type": "mxt:fixed", "value": 100 }`、`{ "type": "mxt:initial_multiplier", "multiplier": 2 }` 或 `{ "type": "mxt:unlimited" }`。方块灵气会额外提高对应资源的有效容量，不占用环境上限。环境优先级为群系 < 维度 < 永久区域 < 阵法覆写。多个玩家共享同一块区块的各资源库存：`random` 随机分配，`equal` 平均分配，`realm_weighted` 按境界权重分配。
...
| `type` | `Holder<resource>` | **必填** | 消耗和释放的资源类型；元素只通过资源的 `aura_type` 参与类型判断。 |
| `aura` | `NumberProvider` | **必填** | 首次处理一件物品时写入其 `mxt:item_aura.remain` 的总量。 |
...
| `max_bonus` | `Map<Holder<resource>, NumberProvider>` | `{}` | 各资源的范围内区块灵气上限加成；重叠阵法按资源分别处理。 |
