...
| `mxt:flight` | `speed`（必填 `NumberProvider`）、`costs`（`ResourceCost[]`，默认 `[]`）、`display`（载具怎么画，默认＝"放平 + 剑刃朝前 + 两倍大"，见下） | 载人飞行：`speed` 是飞行载具速度，`costs` 是每 tick 的消耗。**一个定义最多一条。** 它同时是一个**法器技能**（`ToggableArtifactAbility` 的实现者），所以会作为一格出现在轮盘上：按一下起剑、再按一下落剑（见下）。 |
