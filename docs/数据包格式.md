...
体质不绑定元素；元素相关逻辑应写在灵根或环境配置中。这条边界是**执行**的而不是约定：`element`、`element_affinity`、`element_ability_modifier`、`conflicting_elements`、`overcomes`、`adapted_to`、`damage_types`、`cultivation_multiplier` 这些键一旦出现在体质定义里，**整份包在加载期就会被拒绝并指名那个字段**。（`RecordCodecBuilder` 默认会把不认识的键悄悄丢掉，于是"看着有元素、实际什么都没有"的体质会一路跑下去——所以这里宁可报错。）
