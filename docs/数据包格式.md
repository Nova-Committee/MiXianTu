...
**自我 HUD 的两列是可拖动的**：`mxt:self_hud` 的每一条按 `anchor` 落进左列或右列，两列各自是 `screen.overlay.hud` 框架里的一个元素，**按窗口比例**把位置存在客户端配置 `config/mxt/mxt-hud.json` 的「HUD 布局 → 元素位置」下（键 `resource_bars.left` / `resource_bars.right`，值形如 `0.4375,0.9,true`）。所以玩家能在游戏里（按键 `key.mxt.hud_layout`，默认右 Shift，或客户端命令 `/hud open`）拖动整列，分辨率与 GUI 缩放变化后布局不漂。**列宽与列高由条自己算**（最宽的一条、各条高度之和），数据包不需要声明尺寸，也没有可配置的位置字段。
...
| 层 | 在哪结算 | 读什么 |
| --- | --- | --- |
| 第一层：出力 | 发伤害的那一刻，攻击方一侧 | 定义里写的基础值 × 这次施放提供的 `damage_multiplier` × 这次施放的 `element_modifier`（匹配灵根的 `element_ability_modifier`）× 攻击者在效体质的 `damage_dealt_multiplier` × **这一击的元素**对目标灵根元素的 `overcomes` |
