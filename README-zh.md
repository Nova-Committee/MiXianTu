...
| 模块 | 状态 | 主要内容 |
| --- | --- | --- |
| 注册表、Codec 与 Holder | 完成 | 32 个原版数据包注册表、`mxt:disabled` 标签、加载期 Holder 引用、安全集合 Codec 与固有 `MapCodec` 分派。 |
...
| 元素、灵根、体质、功法与称号 | 制作中 | 授予、条件、修炼倍率、能力和属性已接入；稀有度、功法等级、称号自动解锁等仍是元数据或缺少消费者；灵根分组与筛选由原版标签承担。 |
| 技能与诅咒 | 制作中 | Ability 的事件、资源、冷却、动作与网络已完成；元素亲和未统一结算。Curse 已完成附件和周期调度。 |
| 灵气查询与燃料 | 完成 | `AuraService`、方块灵气、物品灵气、缓存、按 `aura.sync_interval`（默认 5 tick）同步、服务端粒子和隐藏燃料条已闭环。 |
