...
2. 用 `item_binding`、`weapon_binding`、`pill_binding`、`item_aura`、`currency` 等表把现有物品接入框架。
3. 用 `resource`、`realm_stage`、`cultivate_action`、`ability`、`formation` 等表定义规则。
4. 用原版标签为物品、方块、生物类型和 MiXianTu 动态表分组、筛选或临时停用内容。

## 模块完成情况

状态说明：**已接入** 表示数据结构、服务端运行时和测试数据已连通；
**预留** 表示底层能力已存在但尚未作为完整玩法或界面启用；
**内容侧** 表示框架提供读取或执行能力，具体内容由数据包、KubeJS 或附属模组负责。

| 模块        | 状态       | 主要内容                                                                                    |
|-----------|----------|-----------------------------------------------------------------------------------------|
| 资源与资源条    | 已接入      | `resource` 支持上下限、再生、修为与蓝条双向转换、内联 `bars`；内置 `mxt:boss_bar` 渲染器、锚点和排序。                    |
| 境界与修炼     | 已接入      | `realm_stage` 是强绑定单一资源的线性境界链；`cultivate_action`、`cultivation_technique` 负责修炼条件、周期结算和功法。 |
| 灵根、体质与称号  | 已接入      | `element`、`spirit_root`、`physique`、`title` 分别处理元素关系、灵根、独立体质增益和称号属性。                     |
| 技能、行为与条件  | 已接入      | `ability` 配合实体、双实体、方块、物品四类 Action/Condition 固有类型表；支持触发器、冷却、资源消耗、公式和被动属性。                |
| 诅咒与天劫     | 已接入      | `curse` 有附件、调度器、行为与战利品函数接入；`tribulation` 支持分阶段突破结算。                                     |
| 灵气环境      | 已接入      | `aura_zone`、`block_aura`、`item_aura` 提供区块缓存、噪声/波动、元素灵气、方块贡献和修炼燃料。                       |
| 阵法        | 已接入      | `formation` 支持结构校验、激活/周期/失效行为、范围实体行为、维护资源与局部灵气覆写。                                       |
| 锻造        | 已接入      | `forging_method` 和 `forging_blueprint` 支持锻打偏移、成功区间、末 1-6 步匹配、最优步数与品质结算。                 |
| 炼丹、灵植与材料  | 已接入      | `alchemy_recipe`、`spirit_herb`、`material_grade` 支持炼制环境、现有物品绑定和通用品级元数据。                  |
| 法器与数据物品绑定 | 已接入      | `item_archetype` 处理法器存储、飞行、能力和精炼接口；三种物品绑定表把通用物品、武器和丹药分别接入。                              |
| 生物与契约     | 已接入      | `creature_profile`、`contract_type` 支持灵气倾向、内丹、契约、跟随、战斗和召回。                               |
| 秘境与运行时维度  | 已接入 / 预留 | `realm_instance` 已负责进入、返回、人数和时长规则；`RuntimeDimensionService` 可按需加载/卸载维度，尚未自动接管秘境实例。      |
| 宗门与经济     | 已接入      | `sect` 提供成员、贡献、职级、任务、兑换和领地服务；`currency` 支持物品或物品标签货币、找零、交易站和提示。                          |
| 徽章        | 预留       | `badge` 与 `badge_type` 支持空、贴图、提示、按键和配方徽章；当前不主动绑定选择界面或图鉴。                                |
| 通用物品      | 已接入      | 本体提供灵石、灵铁、灵木、空白/未知物品、契约卷轴、御兽铃、灵兽袋、阵盘、秘境令牌、灵力容器、鉴定镜、令牌与制符材料。                             |
...
- **物品到玩法**：框架不以动态表创建新物品。绑定表统一匹配已注册物品或原版物品标签，使
  KubeJS、原版和其他模组物品都能接入武器、丹药、资源、货币与灵气燃料玩法。
...
不要依赖标签 `values` 的顺序。物品和方块匹配字段通常允许单个 ID、单个标签或二者混合的数组；完整字段和限制以 Codec 文档为准。
...
| [物品绑定](docs/item-bindings.md)     | `item_binding`、`weapon_binding` 和 `pill_binding` 的字段。 |
