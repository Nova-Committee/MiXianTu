...
> 变更记录
> - 2026-09-17：**注册表与类型改名**（用户："`cultivation_technique` 改成 `technique`"）：`mxt:cultivation_technique` → `mxt:technique`，数据目录与标签目录同步改为 `mxt/technique/`，类型 `CultivationTechnique` → `Technique`（类移入 `data/cultivation/Technique.java`），`MxtResourceKeys.CULTIVATION_TECHNIQUE` → `TECHNIQUE`，语言键前缀 `cultivation_technique.<ns>.<path>` → `technique.<ns>.<path>`。理由与"为什么 `Aura` 走而 `Technique` 留在 `data/cultivation`"见 `research/audit/resource-cultivation-split.md` §7.3。
> - 2026-09-12：新增本文件。
...
> - 2026-09-13：**补齐熟练度晋升运行时并做成数据驱动**（§8.0.1）：`SkillStage` 加 `mastery`（到达该级所需熟练度，链上非递减由缓存校验）、`Technique` 加 `mastery_resource`、`SpiritIdentityAttachment` 加 `technique_stages`、新增 `TechniqueMasteryService`（晋升 + 发布 `mxt:technique_stage`）、`CultivationGrantService` 按当前水平累积授予。至此"没有玩家当前水平状态"不再是缺口。
...
> - 2026-09-13：**把图标统一成一套**（见 §14）：`HotbarIcon` 提升为双端通用 `IconReference`（内部 `Either<Identifier, ItemStackTemplate>`），新增客户端渲染 Helper `IconRenderer`，`ability`/`resource`/`badge`/`forging_method`/`technique` 全部改用它，并消掉热键栏与热键配置界面里重复的那段渲染逻辑。
...
- `Technique` 新增 `default_stage`（可选，链入口）与 `configuration`（`Map<Holder<skill_stage>, StageConfiguration>`；条目 = 必填 `condition` + 选填 `ability`）。为了让链可达，`configuration` 非空而 `default_stage` 缺失会在解析期报错。
...
- `Technique` 新增 `mastery_resource: Optional<Holder<Resource>>`（缺 `default_stage` 而写了它在解析期报错）。它只回答"用什么衡量"，数值怎么涨由内容包决定：`mxt:trigger` 规则、修炼档案或脚本都行——这就是"数据驱动"的落点，模组不认识熟练度是什么。
- 状态存储：`SpiritIdentityAttachment.technique_stages: Map<Holder<Technique>, Holder<SkillStage>>`（JSON `technique_stages`）。键是功法而非链，因为链可以共享而"这条链上我爬到哪"属于持有者；**没有记录 = 仍在 `default_stage`**，所以入口级不需要写入，旧存档也不会缺字段。
...
- 钩子：`AbilityEventBridge` 的 `% 20 == 0` 周期块里调用 `tick`；同时修炼档案的回复循环改为只遍历 `mxt:aura` 档案并跳过没有存储条目的数值（不再遍历全部 resource 数值，见 `research/audit/resource-cultivation-split.md`）。
...
- 推进逻辑 `runtime/cultivation/CultivationService.java`：
  - `next()`（`:130-141`）从当前级沿 `next_realm` 走**一跳**，并强制链身份一致（链以 `mxt:aura` 档案为键，`RealmStage.cultivation` 指向档案）；凡人态改用该档案的 `first_realm` 与 `start_exp`（2026-09-12 从 `Resource` 迁出，见 `research/audit/resource-cultivation-split.md`）。
...
- 原 `research/13_Mine-And-Slash功能参考.md:159`（该稿已于 2026-09-22 删除）："职业等级本身可以不移植，后续可替换成境界、熟练度或功法等级。"
...
7. 展示层：**功法列表界面已完成（§12）**——图标槽位、等级（名 + 序号）、两种口径的熟练度进度条都已就绪；剩下的是给 `technique` 加 icon 字段并在 `TechniquePanelScreen` 的 `//TODO::` 处叠图。tooltip 展示（T2 之外的等级/倍率/能力列表）仍未做。
...
`SpiritIdentityAttachment`（`learned_techniques` + `technique_stages`）与 `ResourceHolderAttachment` 都是 `ShouldSyncAttachment`，`mxt:technique` / `mxt:skill_stage` 也是客户端同步注册表，所以等级与进度在客户端直接算。
